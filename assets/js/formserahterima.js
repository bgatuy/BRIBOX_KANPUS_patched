...
// cari/buat file JSON dalam folder data/
async function findOrCreateJsonInData(name, initial={items:[],updatedAt:new Date().toISOString()}) {
  await requestDriveToken();
  const { dataFolder } = await ensureUserFolders();

  const q = encodeURIComponent(`'${dataFolder}' in parents and trashed=false and name='${name.replace("'", "\\'")}'`);
  const list = await (await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name)`, {
    headers: { Authorization: `Bearer ${getAccessToken()}` }
  })).json();
  if (list.files?.length) return list.files[0].id;

  // create empty json
  const meta = { name, parents: [dataFolder], mimeType: 'application/json' };
  const boundary = 'bdjson';
  const body = new Blob([
  ...
  const r = await fetch(`https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id`, {
    method: 'POST', headers: { Authorization: `Bearer ${getAccessToken()}` }, body
  });
...
// === append draft serah-terima (dipanggil dari Trackmate) ===
export async function appendSerahTerimaDraft({ driveFileId, fileName, sha256, lokasiDivisi, extractedText }) {
  const m = activeMonth();
  const fname = `serahterima-drafts-${m}.json`;
  const fileId = await findOrCreateJsonInData(fname);
  const payload = await loadJsonFile(fileId);
  ...
  await saveJsonFile(fileId, payload);
  return true;
}
